/* altor: Glailson Miranda da Concei‡Æo
 *
 * Version: 1.0
 *
 * Todos os direitos rezervados a pessoa do Sr. Glailson Miranda
 * Analista de Sistema Sˆnior
 */
...
function dayMouseOut(ev) {
  ...
  if (isRelated(el, ev) || _C) return;
  removeClass(el, "hilite");
  el.calendar.tooltips.firstChild.data = "Selecione uma data";
  stopEvent(ev);
}
...
function Calendar(mondayFirst, dateStr, yourHandler, yourCloseHandler) {
  ...
  this.displayWeekdays = function () {
    ...
    for (var i = 0; i < 7; ++i, cell = cell.nextSibling) {
      ...
      if (!i)
        if (this.mondayFirst) cell.ttip = " Orderna pelo domingo";
        else cell.ttip = "Orderna pela segunda-feira.";
      if (i == SUN || i == SAT) addClass(cell, "weekend");
      cell.firstChild.data = _DN[i + 1 - MON].substr(0,3);
    }
  }
  ...
  hh("-", 1, 100).ttip = "Organizar o primeiro dia da semana.";
  this.title = hh("", 5, 300);
  this.title.className = "title";
  this.title.ttip = "Clique para mover.";
  this.title.style.cursor = "mover";
  hh("X", 1, 200).ttip = "Fechar";

  row = document.createElement("tr");
  thead.appendChild(row);
  row.className = "headrow";

  hh("&#x00ab;", 1, -2).ttip = "Ano anterior (Clique aqui)";
  hh("&#x2039;", 1, -1).ttip = "Mês anterior (Clique aqui)";
  hh("Hoje", 3, 0).ttip = "Dia de Hoje";
  hh("&#x203a;", 1, 1).ttip = "Proxímo mês (Clique aqui)";
  hh("&#x00bb;", 1, 2).ttip = "Proxímo ano (Clique aqui)";
  ...
  cell = hh("Select date", 7, 300);
  cell.className = "ttip";
  cell.ttip = "Clique para mover";
  cell.style.cursor = "mover";
  this.tooltips = cell;
