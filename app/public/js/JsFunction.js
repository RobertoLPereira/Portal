function TrataCheckBox(ComponenteChkAll, ComponentesChk)
{  //alert(ComponenteChkAll+" "+ComponentesChk);
   /*var objCheckBox = document.forms[0].elements["checkboxTela"]; 
   alert("qtd="+objCheckBox.length);
   for (i=0; i < objCheckBox.length; i++) {
       alert (objCheckBox[i].valueOf());
   }*/
}
...
{//debugger
    ...
    for (var i = 0; i < form.elements.length; i++){
        if (form.elements[i].type==='checkbox'){
            //alert("name= "+form.elements[i].name+"  encontra= "+form.elements[i].name.indexOf(ComponentesChk));
...
{   //alert("CheckChanged="+ComponenteChkAll);
    //alert(ComponentesChk);
...
function ControlFrame(NomeFrame, NomePagina)
		{
			top.frames[NomeFrame].location = NomePagina;
		}
function PriRedirect(page, prm, prmTarget)
{
	if(prmTarget !== "")	
		parent.principal.location.removeAttribute(prmTarget);
		parent.principal.location.action = page + prm;
		parent.principal.location.submit();
}
function TabParcela()
{
